...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--alias", default="mac-local", help="Display name for the single local model.")
    parser.add_argument("--secret", default=DEFAULT_SECRET)
    parser.add_argument("--temperature", type=float, default=0.0)
    parser.add_argument("--timeout", type=int, default=180)
    parser.add_argument("--per-category-limit", type=int, default=3, help="Prompts per category. Default: 3")
    parser.add_argument("--delay-seconds", type=float, default=1.5, help="Delay between requests. Default: 1.5")
    parser.add_argument("--output", type=Path, default=ROOT / "results" / "mac_lite_results.csv")
    parser.add_argument("--leaderboard", type=Path, default=ROOT / "results" / "mac_lite_leaderboard.csv")
    parser.add_argument(
        "--category-metrics",
        type=Path,
        default=ROOT / "results" / "mac_lite_category_metrics.csv",
    )
    parser.add_argument("--figures-dir", type=Path, default=ROOT / "figures" / "mac_lite")
    parser.add_argument("--report-dir", type=Path, default=ROOT / "report" / "mac_lite")
    parser.add_argument("--resume", action="store_true", help="Resume from the existing mac-lite results file.")
...
def main() -> int:
    args = parse_args()
    model = ModelSpec(alias=args.alias, model_name=args.model, notes="Single low-load macOS local model")
    ...
    print(
        f"Mac-lite run complete: {stats['written_rows']} rows written, "
